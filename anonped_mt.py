# Multi-threaded version of anonymous pedestrian tracking and recording
...
# Queue for communication between main thread and file writer thread
data_queue = queue.Queue()

# Function to handle writing data to file
...
thread = threading.Thread(target=file_writer, args=(file_name, data_queue))
thread.start()
...
try:
        ...
        if success:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                if cls == 0:  # Filter to detect only people (class ID 0)
                    ...
                    # Store tracking data in the queue
                    current_time_ms = int(time.time() * 1000)
                    data = f'{current_time_ms},{track_id},{float(box[0])},{float(box[1])}\n'
                    data_queue.put(data)
    ...
    data_queue.put(None)  # Signal the file writer thread to exit
    thread.join()
